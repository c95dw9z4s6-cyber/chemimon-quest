...
- バージョン: v6.1
- 状態: リリース候補。Firebase CLI再ログイン完了までは本番v6.0を維持し、v6.1を公開しない。
- リリース名: Stage 10 タイムアタック
...
- 5の倍数Stage節目BGM V3: `assets/audio/chemion-milestone-stage-bgm-v3.mp3`
- Stage 10 Au専用V16 loop: `assets/audio/chemion-stage10-au-boss-v16-loop.mp3`
...
> Chemion Quest v6.1の続きです。添付した一式ZIPを最新版の基準にしてください。NEW_CHAT_HANDOFF.mdと監査仕様を維持し、指定した次版を実装してください。生成済みindex.htmlを直接編集せず、config・data・srcを編集してnpm run releaseを実行し、検査済みのindex.html、一式ZIP、README、監査レポートをください。
...
## v6.1完了時の将来引継ぎ（先行実装禁止）

- V3は5の倍数Stageの節目BGM方針。v6.1ではStage 10だけに使用し、Stage 5のV3化はv6.5で行う。
- Stage 10はWave 1〜9がV3、Au形成完了後がV16。Stage 15以降の専用BGM方針は未確定。
- v7.0後は連続更新を一度停止する。Stage 11〜15構想は保持するが、Stage 15／20のboss順は未確定。
- 敵側王水案とプレイヤー側Au案は将来構想だけで、v6.1へ実装しない。
- Pt、通常変異体、新調製物、装備、critical、rare研究card、Stage間素材、対人戦、multi要素を指示なしに先行実装しない。
